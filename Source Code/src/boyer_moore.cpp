...
void badCharHeuSearch(search_params &sp, vector<int> &indices) {
    ...
    int offset = sp.start;
    while (offset <= (sp.end - sp.search_seq_size)) {
        int patternIndex = sp.search_seq_size - 1;
        ...
        if (patternIndex < 0) {
            ...
            if (offset + sp.search_seq_size < sp.end) {
                offset += sp.search_seq_size - searchItem(input_vector.at(offset + sp.search_seq_size), hashTable);
            }
            else {
                offset += 1;
            }
        } else {
            int diff = patternIndex - searchItem(input_vector.at(offset + patternIndex), hashTable);
            if (diff <= 0) {
                offset += 1;
            }
            else {
